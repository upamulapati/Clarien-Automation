...
export class PaymentOrderPage {
  ...
  async clickFetchCharges() {
    try {
      ...
      // Dump HTML to see the actual button structure
      try {
        const html = await finw.content();
        fs.writeFileSync('debug-fetch-button.html', html, 'utf8');
        console.log('Dumped HTML to debug-fetch-button.html');
      } catch (e) {
        console.log('Failed to dump HTML:', e);
      }
      
      // Find all buttons on the page
      const allButtons = await finw.evaluate(() =>
        Array.from(document.querySelectorAll('input[type="button"], input[type="submit"], button'))
          .map((el: any) => ({ id: el.id, name: el.name, value: el.value, text: el.innerText, type: el.type, outer: el.outerHTML.slice(0, 200) }))
      );
      console.log('All buttons on page:', JSON.stringify(allButtons.slice(0, 20))); // First 20 buttons
      
      // Specifically look for fetch-related buttons
      const fetchBtns = allButtons.filter((b: any) => 
        (b.value || b.text || '').toLowerCase().includes('fetch') || 
        (b.name || '').toLowerCase().includes('fetch')
      );
      console.log('Fetch buttons found:', JSON.stringify(fetchBtns));
      
      if (fetchBtns.length > 0) {
        const btn = fetchBtns[0];
        const selector = btn.id ? `#${btn.id}` : (btn.name ? `input[name="${btn.name}"]` : 'button');
        await finw.locator(selector).first().scrollIntoViewIfNeeded();
        await finw.locator(selector).first().click();
