...
test.describe("CIF Corporate Modification Maker - Address (TC_008)", () => {
  ...
  test("Delete Mailing address and add a new address via POM", async ({ page }) => {
    test.setTimeout(300000);
    const cif = new CrmCorporateModificationPage(page);
    await cif.loginAsMaker();
    const ok = await cif.modifyAddressWorkflow();
    expect(ok, "Mailing address should be deleted and a new address added").toBeTruthy();
  });

  test.skip("TC_008 - Delete Mailing address and add a new address", async ({ page, context }) => {
    ...
    const HOUSE_NO = process.env.HOUSE_NO || "14-126";
    const STREET_NO = process.env.STREET_NO || "14A";
    const STREET_NAME = process.env.STREET_NAME || "DAVIDS STREET";
    const POSTAL_CODE = process.env.POSTAL_CODE || "PI 09";
