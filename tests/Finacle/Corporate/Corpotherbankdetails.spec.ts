...
const CIF_ID = process.env.CIF_ID || "0001000516";
...
const AC_ID = process.env.AC_ID || "666666666";
const CHANNEL = process.env.CHANNEL || "EMAIL";
const ADDR_LINE1 = process.env.ADDRESS_LINE1 || "SYENT STREET 08";
...
async function login(page: Page, userId: string, password: string) {
  ...
  // TC_001.5: re-login if "User already login" appears.
...
test.describe("CIF Corporate Other Bank Details Maker Test Suite", () => {
  ...
  test("Add Corporate Other Bank Details maker workflow", async ({ page, context }) => {
    ...
    // TC_002.5: click Submit if a lingering confirmation remains.
    ...
    console.log("✓ TC_003: Corporate Search Criteria displayed");
    ...
    const expectedColumns = [
      ...
      "Corporate Name", "City", "Unique ID", "Unique ID Type", "Primary SOL ID", "Start from Date", "Status",
    ];
    let columnsFound = 0;
    ...
    // ---------- TC_007: Open Edit Window via right-click >> Edit >> Financial Details >> Other Bank Details ----------
    console.log("TC_007: Right-click on CIF link and select Edit >> Financial Details >> Other Bank Details...");
    const cifLink = resultFrame.locator(`a:has-text("${CIF_ID}")`).first();
    await cifLink.click({ button: "right" });
    await page.waitForTimeout(1500);
    await page.screenshot({ path: "test-results/corp-obd-context-menu.png", fullPage: true }).catch(() => {});
    ...
    // Hover a menu label across all frames (real hover + mouseover dispatch) so
    // its flyout submenu opens and stays open. Returns the hosting frame.
    async function hoverByText(re: RegExp): Promise<Frame | null> {
      for (const f of page.frames()) {
        const it = f.getByText(re).first();
        if (!(await it.isVisible().catch(() => false))) continue;
        await it.scrollIntoViewIfNeeded().catch(() => {});
        await it.hover().catch(() => {});
        await it
    ...
    // Diagnostic: dump the visible context-menu item labels across frames.
    async function dumpMenu(tag: string) {
      for (const f of page.frames()) {
        const items = await f
          .evaluate(() => {
            ...
            return Array.from(document.querySelectorAll("a, td, span, div"))
              ...
              .filter((t) => t && t.length < 30 && /^(View|Edit|Quick Edit|General Details|Financial Details|Other Bank Details|Contact|Phone|Address|KYC)/i.test(t));
          })
          .catch(() => [] as string[]);
        if (items.length) console.log(`[menu ${tag} ${f.url().slice(-22)}]`, JSON.stringify([...new Set(items)]));
      }
    }

    await dumpMenu("after-right-click");

    // Walk the corporate flyout: Edit -> Financial Details (a MENU ITEM).
    // IMPORTANT: For corporate, "Other Bank Details" is NOT a context-menu item.
    // Clicking "Financial Details" opens the Financial window (CorpCreateDetWizard)
    // which has tabs: Basic Info | Financial | Transaction Plan | Other Bank Details.
    const FIN_RE = /^\s*Financial\s*Details\s*$/i;
    let finItem = await findMenuItem(FIN_RE);
    for (let tries = 0; tries < 8 && !finItem; tries++) {
      await hoverByText(/^\s*Edit\s*$/i);
      await page.waitForTimeout(900);
      if (tries === 0) await dumpMenu("after-edit-hover");
      finItem = await findMenuItem(FIN_RE);
      if (!finItem && tries % 2 === 1) {
    ...
    await page.screenshot({ path: "test-results/corp-obd-edit-submenu.png", fullPage: true }).catch(() => {});
    console.log(`TC_007: Edit flyout open (Financial Details visible) = ${!!finItem}`);

    // Click "Financial Details" (its EditAccount handler) to open the Financial window.
    const popupPromise = context.waitForEvent("page", { timeout: 12000 }).catch(() => null);
    if (finItem) {
      await finItem.loc.scrollIntoViewIfNeeded().catch(() => {});
      const fired = await finItem.loc
        .evaluate((el: HTMLElement) => {
          let node: HTMLElement | null = el;
        ...
        .catch(() => false);
      console.log(`TC_007: Financial Details handler fired = ${fired}`);
    }
    ...
    // Locate the Financial window (its tabs include "Other Bank Details").
    const finFormRe = /Other Bank Details|Relationship with Other Banks|Basic Info|Transaction Plan|Save Financial/i;
    let editPage: Page = page;
    let finFrame: Frame | null = null;
    if (popup && !popup.isClosed()) {
      finFrame = await findFrameByText(popup, finFormRe, 8000);
      if (finFrame) editPage = popup;
    }
    if (!finFrame) {
      for (const p of context.pages().filter((x) => !x.isClosed())) {
        const fr = await findFrameByText(p, finFormRe, 4000);
        if (fr) {
          finFrame = fr;
          editPage = p;
          break;
    ...
    await editPage.screenshot({ path: "test-results/corp-financial-window.png", fullPage: true }).catch(() => {});
    expect(finFrame, "Financial window must open").not.toBeNull();
    console.log(`TC_007: Financial window opened (frame=${finFrame!.url().slice(-50)})`);

    // Click the "Other Bank Details" tab inside the Financial window.
    let tabClicked = false;
    for (let tries = 0; tries < 5 && !tabClicked; tries++) {
      for (const f of editPage.frames()) {
        const tab = f
          .locator("a, td, span, div")
          .filter({ hasText: /^\s*Other\s*Bank\s*Details\s*$/i })
          .first();
        if (await tab.isVisible().catch(() => false)) {
          await tab.scrollIntoViewIfNeeded().catch(() => {});
          await tab.click({ timeout: 4000 }).catch(() => {});
          tabClicked = true;
          console.log(`TC_007: 'Other Bank Details' tab clicked in ${f.url().slice(-40)}`);
          break;
        }
      }
      if (!tabClicked) await editPage.waitForTimeout(800);
    }
    console.log(`TC_007: Other Bank Details tab clicked = ${tabClicked}`);
    await editPage.waitForTimeout(2000).catch(() => {});

    // Resolve the Other Bank Details frame (hosts "Relationship with Other Banks" + Add Bank Details).
    let obdFrame: Frame | null =
      (await findFrameByText(editPage, /Relationship with Other Banks|Add Bank Details/i, 8000)) || finFrame;
    await editPage.screenshot({ path: "test-results/corp-obd-window.png", fullPage: true }).catch(() => {});
    expect(obdFrame, "Other Bank Details tab must open").not.toBeNull();
    console.log(`✓ TC_007: Other Bank Details tab opened (frame=${obdFrame!.url().slice(-50)})`);
    ...
    try {
      const dump = await bankFrame
        ...
        .catch(() => null);
      if (dump) fs.writeFileSync("test-results/corp-obd-bankform-dom.json", JSON.stringify(dump, null, 2));
      console.log("TC_008: bank-form DOM dumped to test-results/corp-obd-bankform-dom.json");
    } catch {}
    ...
    if (bankLookup && !bankLookup.isClosed()) {
      ...
      try {
        ...
        fs.writeFileSync("test-results/corp-obd-banklookup-dom.json", JSON.stringify(popDump, null, 2));
        console.log("TC_010: bank lookup DOM dumped to test-results/corp-obd-banklookup-dom.json");
      } catch {}
      ...
      // Selecting Bank Name reloads the popup and populates the dependent Branch
      // Name dropdown; wait for that to settle.
      await bankLookup.waitForLoadState("load").catch(() => {});
      await bankLookup.waitForTimeout(2500).catch(() => {});

      // Step 2: select Branch Name (target the LAST select first after reload).
      let branchSel = "";
      {
    ...
    const fillLookupByLabel = async (labelText: string, value: string, fieldCode: string): Promise<string> => {
      ...
      if (pop && !pop.isClosed()) {
            ...
            const did = await f
              .evaluate(() => {
                ...
                const isNext = (el: HTMLElement) => {
                  ...
                  // Exclude first/prev/last-only controls.
        ...
        // Diagnostic: if no row matched, dump the popup's rows.
        if (!rowFound) {
          try {
            ...
            fs.writeFileSync(`test-results/corp-obd-lookup-${fieldCode}-rows.json`, JSON.stringify(rowDump, null, 2));
            console.log(`TC_011: ${labelText} NO row matched; rows dumped to test-results/corp-obd-lookup-${fieldCode}-rows.json`);
          } catch {}
        }
    ...
    await bankPage.screenshot({ path: "test-results/corp-obd-bankform-filled.png", fullPage: true }).catch(() => {});
    ...
    // ---------- TC_012a: Process Selection (CorpKYCApproval) ----------
    ...
    if (procFrame) {
      console.log("TC_012a: Process Selection window detected; choosing CorpKYCApproval...");
      const kycRe = /Corp\s*KYC\s*Approval|CorpKYCApproval/i;
      let chosen = false;
    ...
    await editPage.screenshot({ path: "test-results/corp-obd-submit-success.png", fullPage: true }).catch(() => {});
    ...
    await page.screenshot({ path: "test-results/corp-obd-grid-after-submit.png", fullPage: true }).catch(() => {});
    ...
    console.log(
      `✓ Corporate Other Bank Details maker flow completed: bank=${BANK_NAME}, A/c ID=${AC_ID}, submitted for CIF ${CIF_ID}.`
    );
  });
});
